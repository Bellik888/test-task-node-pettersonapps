...
const registration = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (isUserExist) {
            return res.status(constants_1.HttpCode.CONFLICT).json({ code: constants_1.HttpCode.CONFLICT, message: 'Email os already exist' });
        }
        const data = yield auth_service_1.default.createUser(req.body);
        res.status(constants_1.HttpCode.CREATED).json({ code: constants_1.HttpCode.CREATED, data });
    }
    catch (error) {
        next(error);
    }
...
const login = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!user) {
            return res.status(constants_1.HttpCode.UNAUTHORIZED).json({ code: constants_1.HttpCode.UNAUTHORIZED, message: errors_1.default.UNAUTHORIZED });
        }
        const token = yield auth_service_1.default.getToken(user);
        if (token)
            yield auth_service_1.default.setToken(user.id, token);
        res.status(constants_1.HttpCode.OK).json({ code: constants_1.HttpCode.OK, data: { token } });
    }
    catch (error) {
        next(error);
    }
...
const logout = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        res.status(constants_1.HttpCode.OK).json({ code: constants_1.HttpCode.OK, message: 'SUCCESS' });
    }
    catch (error) {
        next(error);
    }
